...
# 6-3-2 反向后行断言：(?<!表达式)，指在某个位置向左看，表示所在位置左侧不能匹配表达式
s_6_3_2 = """
需要匹配的
$ a = a^2 $
$123$
...
$$$a=b^2 $$$$
"""
print(re.findall('(?<!\$)\$\$[^\$]+\$\$(?!\$)', s_6_3_2_))

## 提取所有人的生日: 提取下列数据中所有人的生日，使用两个分组，第一个分组提取“月”，第二个分组提取“日”。
ss1 = """
需要匹配的
张伟 1996.8.24
王伟 1993年1月2日
...
1 6
"""
print(re.findall('\d{4}[年.-](\d{1,2})[月\-.](\d{1,2})', ss1))
